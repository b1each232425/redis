...
// GzipResponseWriterWithCloseNotify close
...
// GzipHandlerWithOpts options
func GzipHandlerWithOpts(opts ...GzipOption) (func(http.Handler) http.Handler, error) {
	c := &config{
...
// GzipOption options
type GzipOption func(c *config)

// MinSize minimize
func MinSize(size int) GzipOption {
...
func CompressionLevel(level int) GzipOption {
	return func(c *config) {
		c.level = level
	}
}
...
// Content-Type.
func ContentTypes(types []string) GzipOption {
	return func(c *config) {
...
// NotContentTypes specifies a list of content types to compare
...
// By default, responses are gzipped regardless of
// Content-Type.
func NotContentTypes(types []string) GzipOption {
	return func(c *config) {
		c.contentTypes = []parsedContentType{}
		for _, v := range types {
			mediaType, params, err := mime.ParseMediaType(v)
...
func HTBSize(size int) GzipOption {
	return func(c *config) {
		c.htbSize = size
	}
}
